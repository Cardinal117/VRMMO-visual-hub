...
export default function RangedPresentation({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="w-full h-[100dvh] bg-arcane relative flex flex-col items-center justify-center overflow-hidden font-sans text-white">
      ...
      <button 
        onClick={onBack}
        className="absolute top-4 sm:top-8 left-4 sm:left-8 z-50 p-2 rounded-sm border border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10 hover:border-emerald-400 transition-all flex items-center gap-2 font-mono text-[10px] tracking-widest uppercase backdrop-blur-sm bg-black/40 cursor-pointer"
      >
      ...
      <div className="absolute bottom-4 sm:bottom-8 left-0 right-0 flex flex-col sm:flex-row gap-4 justify-between items-center px-4 sm:px-12 z-20 pointer-events-none">
          ...
          <div className="flex gap-1 sm:gap-2 pointer-events-auto flex-wrap justify-center">
            {slides.map((_, i) => (
              <button 
                ...
                className={`w-6 sm:w-12 h-1 rounded-full transition-all duration-500 flex-shrink-0 ${i === currentSlide ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)]' : 'bg-white/20 hover:bg-white/40'}`}
        ...
        <div className="flex gap-4 pointer-events-auto ml-0 sm:ml-12">
          ...
          <button 
            ...
            className="p-2 sm:p-3 rounded-full border border-emerald-500/30 text-emerald-400 hover:text-emerald-400 hover:border-emerald-400 hover:bg-emerald-500/10 hover:shadow-[0_0_15px_rgba(52,211,153,0.3)] disabled:opacity-30 disabled:pointer-events-none transition-all duration-300"
          >
